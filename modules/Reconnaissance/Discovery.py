...
class Discovery:
    ...
    def run_subdomain_enum(self) -> None:
        ...
        if self._is_local_target():
            ...
            RESET = "\033[0m"
            print(
                f"{YELLOW}  [i] Local/private target — skipping external subdomain enumeration.{RESET}"
            )
    ...
    def run_dns_resolution(self) -> None:
        ...
        if self._is_local_target():
            ...
            RESET = "\033[0m"
            print(
                f"{YELLOW}  [i] Local/private target — skipping DNS resolution.{RESET}"
            )
    ...
    def run_origin_ip_discovery(self) -> None:
        ...
        if self._is_local_target():
            ...
            RESET = "\033[0m"
            print(
                f"{YELLOW}  [i] Local/private target — skipping origin IP discovery.{RESET}"
            )
            self.__pages = []
            return
    ...
    def run_dir_busting(
        ...
        if dirpy_bin is None:
            ...
            print(
                f"{YELLOW}  [i] Build it with:  cd modules/Reconnaissance/Dirpy && make build{RESET}"
            )
        ...
        print(
            f"{YELLOW}  ▶ {RESET}Wordlist: {wordlist}  ({word_count} words)  |  Threads: {threads}"
        )
        ...
        for target_url in targets:
            ...
            safe_target = (
                target_url.replace("://", "_").replace(":", "-").replace("/", "_")
            )
            ...
            if os.path.exists(json_file):
                try:
                    ...
                    print(
                        f"{RED}  [!] Failed to parse Dirpy JSON output: {json_file}{RESET}"
                    )
    ...
    def SaveReport(self, output_dir: str | None = None) -> tuple[str, str]:
        ...
        for key, label in section_labels.items():
            ...
            if val is None:
                continue
            lines.append(
                f"┌─ {label} ({'%d entries' % len(val) if isinstance(val, list) else 'captured'})"
            )
            lines.append(tsep)
