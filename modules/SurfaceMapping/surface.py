...
class SurfaceMapper:
    ...
    def _load_recon_data(self):
        ...
        if recon_file.exists():
            ...
            print(
                f"📂 Loaded Recon data: {len(self.base_urls)} URLs, "
    ...
    def _run_endpoint_mapper(self, timeout: float) -> Dict:
        ...
        waf_detected = (
            ...
            .get("summary", {})
            .get("waf_detected", False)
        )

        mapper = EndpointMapper(
            self.base_urls[:10], self.paths, waf_detected=waf_detected  # Limita hosts
        )
    ...
    def _run_form_mapper(self, timeout: float) -> Dict:
        ...
        form_mapper = FormMapper(self.base_urls[:5], all_endpoints)
    ...
    def _export_for_attack_modules(self) -> Dict:
        ...
        return {
            ...
            "tech_context": {
                "cms": self.results.get("technologies", {})
                .get("summary", {})
                .get("all_cms", []),
                "waf": self.results.get("technologies", {})
                .get("summary", {})
                .get("waf_detected", False),
                "frameworks": self.results.get("technologies", {})
                .get("summary", {})
                .get("all_frameworks", []),
            },
    ...
    def _filter_search_endpoints(self) -> List[str]:
        ...
        endpoints = self.results.get("endpoints", {}).get("endpoints", [])
        return [
            ep for ep in endpoints if any(kw in str(ep).lower() for kw in search_kw)
        ]
    ...
    def _generate_summary(self) -> Dict:
        ...
        return {
            ...
            "technologies": {
                ...
                .get("summary", {})
                .get("waf_detected", False),
                "waf_type": self.results.get("technologies", {})
                .get("summary", {})
                .get("waf_type", None),
                "frameworks": self.results.get("technologies", {})
                .get("summary", {})
                .get("all_frameworks", []),
                "servers": self.results.get("technologies", {})
                .get("summary", {})
                .get("all_servers", []),
            },
            "high_value_targets": {
    ...
    def _print_tech_summary(self):
        ...
        if tech:
            ...
            print(
                f"  ├─ Frameworks: {', '.join(tech.get('all_frameworks', [])) or 'None'}"
            )
            print(
                f"  └─ Servers: {', '.join(tech.get('all_servers', [])) or 'Unknown'}"
            )
    ...
    def _print_js_summary(self):
        ...
        print(
            f"  └─ Secrets found: {js.get('total_secrets', 0)} ({js.get('high_severity', 0)} high/critical)"
        )
    ...
    def _print_final_summary(self):
        ...
        print(
            f"    • Admin exposed: {'⚠️  Yes' if risk.get('exposed_admin') else '✅ No'}"
        )
        print(
            f"    • DB exposed: {'🔴 Yes!' if risk.get('exposed_databases') else '✅ No'}"
        )
        print(
            f"    • JS leaks: {'🔴 Yes!' if risk.get('js_secrets_leaked') else '✅ No'}"
        )
        print(f"{'='*60}")
